...
class _GeneralmanagerHomePageState extends State<GeneralmanagerHomePage> {
  final List<String> areas = ['Jadah', 'Dubai'];

  final List<String> propertyTypes = ['APARTMENT', 'VILLA'];
  ...
  bool get _hasPricingPolicyChanges =>
      ...
      listingPriceMargins?.hasChanges == true ||
  ...
  Future<void> _getPricingPolicy() async {
    ...
    setState(() {
      ...
      propertyProfitMargins = _MarginConfig(
        ...
        saleMargin: pricingPolicyModel.sellProfitMargin.toString(),
        rentMargin: pricingPolicyModel.rentProfitMargin.toString(),
      );

      listingPriceMargins = _MarginConfig(
        region: pricingPolicyModel.city,
        propertyType: pricingPolicyModel.propertyType,
        saleMargin: pricingPolicyModel.saleListingMargin.toString(),
        rentMargin: pricingPolicyModel.rentListingMargin.toString(),
      );

      propertyPriceAdjustment = _AdjustmentConfig(
        region: pricingPolicyModel.city,
        propertyType: pricingPolicyModel.propertyType,
        saleSign: _signFor(pricingPolicyModel.saleGlobalAdjust),
        saleAdjustment: pricingPolicyModel.saleGlobalAdjust.abs().toString(),
        rentSign: _signFor(pricingPolicyModel.rentGlobalAdjust),
        rentAdjustment: pricingPolicyModel.rentGlobalAdjust.abs().toString(),
      );
    });
  }

  Future<void> _selectPricingPolicy({
    String? city,
    String? propertyType,
  }) async {
    final nextCity = city ?? _city;
    final nextPropertyType = propertyType ?? _propertyType;

    if (nextCity == _city && nextPropertyType == _propertyType) {
      return;
    }

    setState(() {
      _city = nextCity;
      _propertyType = nextPropertyType;
      propertyProfitMargins = null;
      listingPriceMargins = null;
      propertyPriceAdjustment = null;
      _inputVersion++;
  ...
  PricingPolicyModel? _buildUpdatedPricingPolicyModel() {
    final profitMargins = propertyProfitMargins;
    final listingMargins = listingPriceMargins;
    final priceAdjustment = propertyPriceAdjustment;
    ...
    final sellProfitMargin = double.tryParse(profitMargins.saleMargin.trim());
    final rentProfitMargin = double.tryParse(profitMargins.rentMargin.trim());
    final saleListingMargin = double.tryParse(listingMargins.saleMargin.trim());
    final rentListingMargin = double.tryParse(listingMargins.rentMargin.trim());
    final saleGlobalAdjust = _signedNumber(
      priceAdjustment.saleSign,
      priceAdjustment.saleAdjustment,
    );
    final rentGlobalAdjust = _signedNumber(
      priceAdjustment.rentSign,
      priceAdjustment.rentAdjustment,
    );

    if (sellProfitMargin == null ||
        rentProfitMargin == null ||
        saleListingMargin == null ||
        rentListingMargin == null ||
        saleGlobalAdjust == null ||
        rentGlobalAdjust == null) {
      return null;
    }

    return PricingPolicyModel(
      id: _pricingPolicyModel?.id ?? '',
      city: _city,
      propertyType: _propertyType,
  ...
  double? _signedNumber(String sign, String value) {
    final number = double.tryParse(value.trim());
    if (number == null) {
      return null;
    }

    return sign == '-' ? -number.abs() : number.abs();
  }

  String _signFor(double value) {
    return value < 0 ? '-' : '+';
  }

  void _cancelPricingPolicyChanges() {
    setState(() {
      propertyProfitMargins = propertyProfitMargins?.cancelled();
      listingPriceMargins = listingPriceMargins?.cancelled();
      propertyPriceAdjustment = propertyPriceAdjustment?.cancelled();
      _inputVersion++;
    });
  }
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Padding(
                      padding: EdgeInsets.only(bottom: width * (15 / 1920)),
                      child: Column(
                        children: [
                          Expanded(
                            child: ListView(
                              ...
                              children: [
                                ...
                                if (_hasPricingPolicyChanges) ...[
                                  SizedBox(height: width * (20 / 1920)),
                                  _actionButtons(
                                    width,
                                    onConfirm: _isUpdatingPricingPolicy
                                        ? null
                                        : _updatePricingPolicy,
                                    onCancel: _isUpdatingPricingPolicy
                                        ? null
                                        : _cancelPricingPolicyChanges,
                                    confirmLabel: _isUpdatingPricingPolicy
                                        ? 'Updating...'
                                        : 'Confirm',
                                  ),
                                ],
  ...
  Widget _pricingPolicySelector(double width) {
    return Container(
      padding: EdgeInsets.symmetric(
        horizontal: width * (28 / 1920),
      ...
      child: Row(
        children: [
          ...
          _selectorWithLabel(
            width,
            label: 'Region',
            child: _compactDropDownBox(
              width,
              value: _city,
              items: areas,
              onChanged: (value) => _selectPricingPolicy(city: value),
            ),
          ),
          ...
          _selectorWithLabel(
            ...
            child: _compactDropDownBox(
              ...
              value: _propertyType,
              items: propertyTypes,
              onChanged: (value) => _selectPricingPolicy(propertyType: value),
            ),
          ),
        ],
      ),
